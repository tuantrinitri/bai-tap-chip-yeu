...
@section('page_content')
    ...
    @if (count($contacts) > 0)
        <div class="p-2" style="display: flex;justify-content: flex-end;">
            <a href="javascript:;" class="btn btn-info btn-xs float-right" data-toggle="collapse"
                ...
                <div class="row">
                    <div class="col-2">
                        <div class="form-group">
                            <input type="text" placeholder="{{ trans('contact::admin.pla_sender_name') }}"
                                class="form-control" name="sender_name" aria-describedby="helpId"
                                value="{{ isset($filterdata['sender_name']) ? $filterdata['sender_name'] : '' }}">
                        </div>
                    </div>
                    <div class="col-3">
                        <div class="form-group">
                            <input type="text" placeholder="{{ trans('contact::admin.pla_title') }}" class="form-control"
                                name="title" aria-describedby="helpId"
                                value="{{ isset($filterdata['title']) ? $filterdata['title'] : '' }}">
                        </div>
                    </div>
                    <div class="col-3">
                        <div class="form-group">
                            <input type="text" placeholder="{{ trans('contact::admin.pla_topic') }}" class="form-control"
                                name="topic" aria-describedby="helpId"
                                value="{{ isset($filterdata['topic']) ? $filterdata['topic'] : '' }}">
                        </div>
                    </div>
                    <div class="col-2">
                        <div class="form-group">
                            <select class="form-control" name="status" id="">
                                ...
                                <option value="1"
                                    {{ (isset($filterdata['status']) ? $filterdata['status'] : -1) == 1 ? 'selected' : '' }}>
                                    {{ trans('contact::admin.status_new') }}
                                </option>
                                <option value="2"
                                    {{ (isset($filterdata['status']) ? $filterdata['status'] : -1) == 2 ? 'selected' : '' }}>
                                    {{ trans('contact::admin.status_received') }}</option>
                                <option value="3"
                                    {{ (isset($filterdata['status']) ? $filterdata['status'] : -1) == 3 ? 'selected' : '' }}>
                                    {{ trans('contact::admin.status_reply') }}
                                </option>
                                <option value="4"
                                    {{ (isset($filterdata['status']) ? $filterdata['status'] : -1) == 4 ? 'selected' : '' }}>
                                    {{ trans('contact::admin.status_cancel') }}</option>
                            </select>
                        </div>
                    </div>
                    <div>
                        <button type="submit" class="btn btn-primary" id="button-search-document"><i
                                class="fas fa-filter mr-2"></i>{{ trans('contact::admin.filter') }}</button>
                        <a href="{{ route('contact.admin.list') }}" class="btn btn-danger"><i
                                class="fas fa-trash mr-2"></i>{{ trans('contact::admin.delete') }}</a>
                    </div>
                </div>
            </form>
        </div> --}}
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @foreach ($contacts as $contact)
                            <tr>
                                <td class="text-center"><input name="ids[]" type="checkbox" class="form-check-input-styled"
                                        value="{{ $contact['id'] }}"></td>
                                <td onclick="viewcontact('{{ route('contact.admin.view', $contact['id']) }}');">

                                    {{ $contact['sender_name'] ? $contact['sender_name'] : '-' }}

                                </td>

                                <td onclick="viewcontact('{{ route('contact.admin.view', $contact['id']) }}');">
                                    {{ $contact['sender_email'] ? $contact['sender_email'] : '-' }}</td>
                                <td onclick="viewcontact('{{ route('contact.admin.view', $contact['id']) }}');"
                                    class="text-center">
                                    {{ $contact['sender_phone'] ? $contact['sender_phone'] : '-' }}</td>
                                <td onclick="viewcontact('{{ route('contact.admin.view', $contact['id']) }}');"
                                    class="text-center">
                                    {{ $contact['reply_by'] ? user_display_name($contact['reply_by']) : '-' }}
                                </td>
                                <td onclick="viewcontact('{{ route('contact.admin.view', $contact['id']) }}');"
                                    class="text-center">{!! mod_contact_get_html_status($contact['status']) !!}</td>
                                <td onclick="viewcontact('{{ route('contact.admin.view', $contact['id']) }}');"
                                    class="text-center"><span data-popup="tooltip"
                                        title="{{ $contact['created_at'] }}">{{ cms_time_elapsed_string($contact['created_at']) }}</span>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
